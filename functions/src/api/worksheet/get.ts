...
export const getWorksheetById = https.onCall(
    async (worksheetId: string, context: https.CallableContext) => {
...
async function getDays(worksheetDocRef: admin.firestore.DocumentReference) {
    const daysDocs = await worksheetDocRef.collection('days').get()
